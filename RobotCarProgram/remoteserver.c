...
int main(int argc, char *argv[])
{
  ...
  pthread_t t1, t2;
	...
	}else if(inet_pton(AF_INET, "127.0.0.1", &serv_addr.sin_addr)<=0)
	{
    ...
    while ((n = read(newsockfd, &buffer, BUFFER_SIZE)) > 0)
    {
      ...
      if (time_stamp <2000000) {
        ...
        write(newsockfd, "{\"version\":2}", 13);
          ...
          printf("****************Received Unknown Signal 1\n");
				for (count = 2; count < n; count ++) {
					 PhaseScratchCmd(buffer[count]);
					}
		}else{
          printf("****************Received Unknown Signal 2\n");
			for (count = 0; count < n; count ++) {
...
int send_all(int socket, void *buffer, size_t length)
{
    ...
    while (length > 0)
    {
        int i = send(socket, ptr, length, MSG_DONTWAIT);
        if (i < 1) return 0;
        ptr += i;
        length -= i;
    }
...
void *fun1(void *arg) {
  ...
  float dis = 0, temp_min = 0, temp_max = 0, temp_value = 0, temp = 0;
  {
    ...
    {
      usleep(1);
      char buffer[1024] = {0}, sendBuffer[sizeof(int)*3+2]= {0};
      temp_min  = temp_max;
      temp_value = temp_max;
      for (cnt = 0; cnt < 3; cnt ++ ) {
        temp = disMeasure(); 
      ...
      else  if(read( sock , buffer, 1024) > 0 ){	
        printf("Checking for Stop Signal %s\n", buffer);
        if(strcmp(buffer, "Stop") == 0){
          printf("Requesting the car to stop");
          disWarning = 1;
          if (carstate.forward) {
            if (!(carstate.autoAvoid)) {
              stop();
      ...
      int length = snprintf(sendBuffer, sizeof(sendBuffer), "%f\n", dis);
      printf("Sending Data %s\n", sendBuffer);
      send_all(sock, sendBuffer, length);
      if (!poweroffFlag) {
        beepWarning();
      }
